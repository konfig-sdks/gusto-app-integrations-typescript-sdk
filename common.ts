...
import { readableStreamToString, GustoAppIntegrationsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new GustoAppIntegrationsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new GustoAppIntegrationsError(e, e.response?.data)
                }
                if (innerError instanceof GustoAppIntegrationsError) {
                    // Got "GustoAppIntegrationsError" from the above try block
                    throw innerError;
                }
